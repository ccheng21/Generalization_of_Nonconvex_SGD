...
dataset_size = 50000 # only sample the first 10000 samples
test_dataset_size = 10000
kwargs = {'num_workers': 1, 'pin_memory': True} if args.cuda else {}

transform_train_rotated = transform = transforms.Compose(
    [transforms.ToTensor(),
...
train_loader_aug = torch.utils.data.DataLoader(
    torch.utils.data.ConcatDataset(
        [CIFAR10RandomLabels(corruptprob=args.corrupt_prob, num_classes=10,  root='../../data', train=True, download=True, transform=transform_train),
        CIFAR10RandomLabels(corruptprob=args.corrupt_prob, num_classes=10,  root='../../data', train=True, download=True, transform=transform_train_rotated)]), 
...
train_loader = torch.utils.data.DataLoader(
    CIFAR10RandomLabels(corruptprob=args.corrupt_prob, num_classes=10,  root='../../data', train=True, download=True, 
...
full_train_loader = torch.utils.data.DataLoader(
    CIFAR10RandomLabels(corruptprob=args.corrupt_prob, num_classes=10, root='../../data', train=True, download=True, 
                   transform=transform_train),
    batch_size=dataset_size, sampler=torch.utils.data.sampler.RandomSampler(list(range(dataset_size))), shuffle=False, **kwargs) 
    

test_loader = torch.utils.data.DataLoader(
    CIFAR10RandomLabels(corruptprob=args.corrupt_prob, num_classes=10, root='../../data', train=False, 
...
#        super(Net, self).__init__()
#
#        self.fc1 = nn.Linear(1024,320, bias=False) #784
...
##            var_gradient.append(tmp_var)
    ...
    # save files
    exp_dir = os.path.join('./saved_files', 'cifar10_'+str(args.corrupt_prob)+'_'+str(args.lr)+args.exp_name)
